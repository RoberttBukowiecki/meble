...
import { calculateSnapV3, calculatePartSnapV3CrossAxis, type SnapV3Input, type SnapV3Settings } from './snapping-v3';
...
describe('USER SCENARIO: Part 9 and Part 10', () => {
  ...
  it('analyzes Part 9 (target) face positions', () => {
    ...
    console.log('\nOBB axes:');
    console.log('  axes[0] (width 600):', obb.axes[0].map(n => n.toFixed(2)));
    console.log('  axes[1] (height 400):', obb.axes[1].map(n => n.toFixed(2)));
    console.log('  axes[2] (depth 18):', obb.axes[2].map(n => n.toFixed(2)));

    console.log('\nX-perpendicular faces:');
    const xFaces = faces.filter(f => Math.abs(f.normal[0]) > 0.9);
    for (const f of xFaces) {
      console.log(`  Center: [${f.center.map(n => n.toFixed(1)).join(', ')}], Normal X: ${f.normal[0].toFixed(2)}`);
    }
  ...
  it('analyzes Part 10 (moving, compound rotation) face positions', () => {
    ...
    for (const f of faces) {
      const worldDir =
        Math.abs(f.normal[0]) > 0.9 ? (f.normal[0] > 0 ? '+X' : '-X') :
        Math.abs(f.normal[1]) > 0.9 ? (f.normal[1] > 0 ? '+Y' : '-Y') :
        Math.abs(f.normal[2]) > 0.9 ? (f.normal[2] > 0 ? '+Z' : '-Z') : '?';
      console.log(`  Axis ${f.axisIndex}, sign ${f.sign > 0 ? '+' : '-'}: Center [${f.center.map(n => n.toFixed(1)).join(', ')}] → ${worldDir}`);
    }
  ...
  it('tests snap from Part 10 to Part 9 on X axis', () => {
    ...
    if (result.snapped) {
      ...
      const obb10 = createOBBFromPart({ ...movingPart10, position: result.position as [number, number, number] });
...
describe('Y-Rotation 90° Side Face Snapping', () => {
  ...
  describe('Part with 90° Y rotation - face analysis', () => {
    ...
    it('analyzes face positions for 90° Y-rotated part', () => {
      ...
      console.log('  axes[0] (local X, width 600):', obb.axes[0].map(n => n.toFixed(3)));
      console.log('  axes[1] (local Y, height 400):', obb.axes[1].map(n => n.toFixed(3)));
      console.log('  axes[2] (local Z, depth 18):', obb.axes[2].map(n => n.toFixed(3)));
      ...
      for (const face of faces) {
        ...
        console.log(`  Normal: [${face.normal.map(n => n.toFixed(2)).join(', ')}] → World ${worldDir}`);
