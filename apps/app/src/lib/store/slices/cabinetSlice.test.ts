...
describe('cabinetSlice', () => {
  ...
  it('regenerates cabinet parts when placement changes', () => {
    ...
    const store = createCabinetStore({
      ...
      parts: [oldPart, { ...oldPart, id: 'other', cabinetMetadata: { cabinetId: 'other', role: 'BOTTOM' } }],
    });
  ...
  it('regenerates cabinet when params change with skipHistory', () => {
    ...
    const newParams: CabinetParams = { ...defaultParams, width: 150, topBottomPlacement: 'overlay' };
    const generator = jest.fn((cabinetId: string, furnitureId: string) => [
  ...
  it('removes cabinet and associated parts', () => {
    ...
    const parts: Part[] = [
      { ...makeGeneratedPart('Keep', 'other', 'f-1'), id: 'p-keep', createdAt: new Date('2024-01-01'), updatedAt: new Date('2024-01-01') },
      { ...makeGeneratedPart('Remove', 'cab-3', 'f-1'), id: 'p-remove', createdAt: new Date('2024-01-01'), updatedAt: new Date('2024-01-01') },
    ];
  ...
  it('duplicates cabinet with offset parts and selects the copy', () => {
    ...
    const duplicatedParts = state.parts.filter((p) => p.cabinetMetadata?.cabinetId === duplicated.id);
