...
const createHistoryStore = () =>
  create<HistorySlice>()(
    createHistorySlice as unknown as StateCreator<HistorySlice, [], [], HistorySlice>
  );
...
describe('historySlice', () => {
  ...
  it('trims undo stack while preserving milestones and clears redo stack', () => {
    ...
    const entry2 = createEntry('e2', 2, {
      meta: { id: 'e2', timestamp: 2, label: HISTORY_LABELS.UPDATE_PART, kind: 'geometry', isMilestone: true },
    });
  ...
  it('wraps mutator execution with history entry creation', () => {
    ...
    expect(entry).toMatchObject({
      after: { value: 'result' },
      meta: expect.objectContaining({ id: 'built', timestamp: 10, label: HISTORY_LABELS.MILESTONE, kind: 'misc' }),
    });
