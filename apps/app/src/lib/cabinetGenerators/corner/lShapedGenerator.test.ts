...
const createDefaultParams = (
  overrides?: Partial<CornerInternalCabinetParams>
): CornerInternalCabinetParams => ({
  type: "CORNER_INTERNAL",
  width: CORNER_DEFAULTS.W,
  height: 720,
  depth: CORNER_DEFAULTS.D,
...
describe("generateLShapedCorner", () => {
  ...
  describe("bottom panel", () => {
    ...
    it("should have bottom panel width = W - 2*t for inset mounting", () => {
      ...
      const bottomPart = findPartsByRole(parts, "CORNER_BOTTOM")[0];
      // W - 2*t (18mm thickness)
      const expectedWidth = CORNER_DEFAULTS.W - 2 * 18;
      expect(bottomPart.width).toBe(expectedWidth);
    });
  ...
  describe("side panels", () => {
    ...
    it("should position external side at X = W - t/2", () => {
      ...
      const externalSide = findPartsByRole(parts, "CORNER_SIDE_EXTERNAL")[0];
      // External side at X = W - t/2
      const expectedX = CORNER_DEFAULTS.W - 18 / 2;
      expect(externalSide.position[0]).toBeCloseTo(expectedX, 0);
    });
    ...
    it("should have sides with depth D", () => {
      ...
      const internalSide = findPartsByRole(parts, "CORNER_SIDE_INTERNAL")[0];
      const externalSide = findPartsByRole(parts, "CORNER_SIDE_EXTERNAL")[0];
      expect(internalSide.width).toBe(CORNER_DEFAULTS.D);
      expect(externalSide.width).toBe(CORNER_DEFAULTS.D);
    });
  });
  ...
  describe("back panel", () => {
    ...
    it("should position back panel at Z = D + backT/2 (behind cabinet)", () => {
      ...
      const backPart = findPartsByRole(parts, "CORNER_BACK")[0];
      // Back at Z = D + backT/2 (outside/behind cabinet body)
      const expectedZ = CORNER_DEFAULTS.D + 3 / 2; // backMaterial thickness = 3
      expect(backPart.position[2]).toBeCloseTo(expectedZ, 0);
    });
    ...
    it("should have back panel width = W (overlaps sides)", () => {
      ...
      const backPart = findPartsByRole(parts, "CORNER_BACK")[0];
      // Back panel overlaps sides: width = W
      expect(backPart.width).toBe(CORNER_DEFAULTS.W);
    });
  ...
  describe("front panel and door", () => {
    ...
    it("should not generate front panel or door for NONE type", () => {
      const cornerConfig = CornerDomain.updateFrontType(CornerDomain.createConfig(), "NONE");
  ...
  describe("shelves", () => {
    ...
    it("should have shelf width = W - 2*t", () => {
      ...
      // W - 2*t
      const expectedWidth = CORNER_DEFAULTS.W - 2 * 18;
      expect(shelfPart.width).toBe(expectedWidth);
    });
  });
