...
import { CabinetParams, CabinetMaterials, Material } from "@/types";
...
export function generateWardrobe(
  ...
  for (let i = 0; i < actualDoorCount; i++) {
    const doorCenterX =
      -availableDoorWidth / 2 + singleDoorWidth / 2 + i * (singleDoorWidth + DOOR_GAP);
    parts.push({
      name: `Front ${i + 1}`,
      furnitureId,
      group: cabinetId,
  ...
  if (
    params.decorativePanels &&
    (params.decorativePanels.top?.enabled || params.decorativePanels.bottom?.enabled)
  ) {
