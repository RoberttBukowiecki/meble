...
describe('AuthProvider', () => {
  ...
  it('unsubscribes on unmount', async () => {
    ...
    const subscription =
      mockSupabaseClient.auth.onAuthStateChange.mock.results[0].value.data
        .subscription;
...
describe('AuthProvider - Guest Credit Migration', () => {
  ...
  it('attempts to migrate guest credits when user signs in', async () => {
    // Set up guest session in localStorage
    localStorage.setItem(
      'meblarz_guest_session',
      JSON.stringify({ id: 'guest-session-123' })
    );
