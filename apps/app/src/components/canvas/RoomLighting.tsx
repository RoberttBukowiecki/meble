...
export function RoomLighting() {
  ...
  return (
    <group>
      ...
      {!isSimulation && (
        <>
          <ambientLight intensity={sceneTheme.AMBIENT_INTENSITY} color={sceneTheme.AMBIENT_COLOR} />
          <hemisphereLight
            args={[
              sceneTheme.HEMISPHERE_SKY,
              sceneTheme.HEMISPHERE_GROUND,
              sceneTheme.HEMISPHERE_INTENSITY,
            ]}
            position={[0, 5000, 0]}
          />
        </>
      ...
      {isSimulation && (
        <>
          ...
          <ambientLight
            intensity={sceneTheme.SIMULATION_AMBIENT_INTENSITY}
            color={sceneTheme.AMBIENT_COLOR}
          />
          <hemisphereLight
            args={[
              sceneTheme.HEMISPHERE_SKY,
              sceneTheme.SIMULATION_HEMISPHERE_GROUND,
              sceneTheme.SIMULATION_HEMISPHERE_INTENSITY,
            ]}
            position={[0, 5000, 0]}
          />
          {windowLights}
