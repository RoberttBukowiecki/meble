...
const ParameterForm = ({
  ...
  const getBackOverlapRatio = (): number => {
    if ("backOverlapRatio" in params && params.backOverlapRatio !== undefined)
      return params.backOverlapRatio;
  ...
  const getHangerCutoutsEnabled = (): boolean => {
    ...
    return (
      (params as WallCabinetParams).hangerCutouts?.enabled ?? DEFAULT_HANGER_CUTOUT_CONFIG.enabled
    );
  };
  ...
  const getHangerCutoutHeight = (): number => {
    ...
    return (
      (params as WallCabinetParams).hangerCutouts?.height ?? DEFAULT_HANGER_CUTOUT_CONFIG.height
    );
  };
  ...
  const getHangerCutoutHorizontalInset = (): number => {
    ...
    return (
      (params as WallCabinetParams).hangerCutouts?.horizontalInset ??
      DEFAULT_HANGER_CUTOUT_CONFIG.horizontalInset
    );
  };
  ...
  const getHangerCutoutVerticalInset = (): number => {
    ...
    return (
      (params as WallCabinetParams).hangerCutouts?.verticalInset ??
      DEFAULT_HANGER_CUTOUT_CONFIG.verticalInset
    );
  };
  ...
  const updateHangerCutouts = (updates: Partial<HangerCutoutConfig>) => {
    ...
    const currentConfig =
      (params as WallCabinetParams).hangerCutouts ?? DEFAULT_HANGER_CUTOUT_CONFIG;
    onChange({
      ...params,
  ...
  const getFoldingSplitRatio = (): number => {
    ...
    return (
      (params as WallCabinetParams).foldingDoorConfig?.splitRatio ??
      DEFAULT_FOLDING_DOOR_CONFIG.splitRatio
    );
  };
  ...
  const getFoldingSectionGap = (): number => {
    ...
    return (
      (params as WallCabinetParams).foldingDoorConfig?.sectionGap ??
      DEFAULT_FOLDING_DOOR_CONFIG.sectionGap
    );
  };
  ...
  const updateFoldingDoor = (updates: Partial<FoldingDoorConfig>) => {
    if (params.type !== "WALL") return;
    const currentConfig =
      (params as WallCabinetParams).foldingDoorConfig ?? DEFAULT_FOLDING_DOOR_CONFIG;
    onChange({
      ...params,
  ...
  const drawerHasFronts = params.drawerConfig
    ? Drawer.hasExternalFronts(params.drawerConfig)
    : false;
          ...
          <div className="grid grid-cols-2 gap-4 items-end">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(val: TopBottomPlacement) =>
                  updateParams({ topBottomPlacement: val } as any)
                }
              >
            ...
            <div className="flex items-center justify-between border rounded-md p-2 h-9">
              <Label className="text-sm cursor-pointer flex-1" htmlFor="has-back">
                Plecy (HDF)
              </Label>
              <Switch
                id="has-back"
            ...
            <div className="space-y-2 bg-muted/30 p-3 rounded-md">
              <div className="flex justify-between">
                ...
                <span className="text-xs font-mono">
                  {Math.round(getBackOverlapRatio() * 100)}%
                </span>
              </div>
              <Slider
              ...
              {getCornerConfig()?.frontType === "SINGLE" && (
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(val: CornerDoorPosition) =>
                          updateCornerConfig({ doorPosition: val })
                        }
                      >
                        <SelectTrigger className="h-9">
                          <SelectValue />
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    ...
                    {(getCornerConfig()?.W || 1000) - (getCornerConfig()?.doorWidth || 450) - 40} mm
                  </p>
                </div>
              )}
            </div>
                    ...
                    <div className="bg-muted/30 p-3 rounded-md space-y-3">
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="text-xs font-mono">
                            {Math.round(getFoldingSplitRatio() * 100)}%
                          </span>
                        </div>
                        <Slider
        ...
        <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Ilość półek wewnętrznych: {getShelfCount()}
                  </p>
                </div>
                <div className="w-32">
                  <Slider
          ...
          <ConfigRow
            ...
            description={
              hasInterior
                ? interiorSummary
                : "Użyj konfiguracji półek/szuflad powyżej lub skonfiguruj zaawansowane sekcje"
            }
          ...
          {/* Countertop Configuration - for kitchen and corner cabinets */}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          ...
          {step === "materials" && (
            <div className="space-y-6">
              ...
              {/* Countertop material - for kitchen and corner cabinets with countertop enabled */}
              {(selectedType === "KITCHEN" ||
                selectedType === "CORNER_INTERNAL" ||
                selectedType === "CORNER_EXTERNAL") &&
                params.countertopConfig?.hasCountertop &&
                countertopMaterials.length > 0 && (
                  <div className="space-y-3">
                    <Label className="text-base font-medium">Blat kuchenny</Label>
                    <Select
                      value={
                        params.countertopConfig?.materialId || defaultCountertopMaterialId || ""
                      }
                      onValueChange={(id) =>
                        setParams((prev) => ({
                          ...prev,
                          countertopConfig: {
                            ...prev.countertopConfig,
                            hasCountertop: true,
                            materialId: id,
                          },
                        }))
