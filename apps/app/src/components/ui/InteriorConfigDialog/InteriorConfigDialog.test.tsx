...
describe("InteriorConfigDialog Component", () => {
  ...
  describe("ICD-002: Open dialog with existing config", () => {
    it("displays existing zones correctly", () => {
      render(<InteriorConfigDialog {...defaultProps} config={createMultiZoneConfig()} />);
  ...
  describe("ICD-003: Add new zone (section)", () => {
    it("adds zone when clicking add button", async () => {
      ...
      // Find and click "Dodaj sekcję" button - get all and use the main panel one
      const addButtons = screen.getAllByRole("button", { name: /dodaj sekcję/i });
      // Use the first button (main add section button in panel)
  ...
  describe("ICD-008: Doors + drawer fronts conflict warning", () => {
    ...
    it("does not show warning when no drawer fronts", () => {
      render(
        <InteriorConfigDialog {...defaultProps} config={createShelvesConfig()} hasDoors={true} />
      );
  ...
  describe("ICD-012: Breadcrumb navigation", () => {
    it("shows breadcrumb for current position", () => {
      render(<InteriorConfigDialog {...defaultProps} config={createShelvesConfig()} />);
  ...
  describe("ICD-013: Navigate into nested zone", () => {
    it("updates breadcrumb when entering nested zone", async () => {
      ...
      render(<InteriorConfigDialog {...defaultProps} config={nestedConfig} />);
  ...
  describe("Dialog state management", () => {
    it("resets local state when dialog opens", async () => {
      ...
      const { rerender } = render(
        <InteriorConfigDialog {...defaultProps} open={false} onOpenChange={onOpenChange} />
      );

      // Dialog should not be visible
      expect(screen.queryByText("Konfiguracja wnętrza szafki")).not.toBeInTheDocument();

      // Rerender with open=true
      rerender(<InteriorConfigDialog {...defaultProps} open={true} onOpenChange={onOpenChange} />);
  ...
  describe("Footer buttons", () => {
    it("renders cancel and save buttons", () => {
      ...
      expect(screen.getByRole("button", { name: /zastosuj zmiany/i })).toBeInTheDocument();
    });
  });
});
