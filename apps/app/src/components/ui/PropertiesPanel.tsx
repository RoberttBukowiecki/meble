      ...
      <AccordionContent className="pb-4 pt-0">
        <div className="space-y-3 px-1">
          <p className="text-xs text-muted-foreground">
            {hasConfig ? `Skonfigurowane: ${summary}` : "Brak skonfigurowanych frontów bocznych"}
          </p>
          <Button
            variant="outline"
            size="sm"
  ...
  return (
    <AccordionItem value="interior" className="border-b-0">
      ...
      <AccordionContent className="pb-4 pt-0">
        <div className="space-y-3 px-1">
          <p className="text-xs text-muted-foreground">
            {hasConfig ? `Skonfigurowane: ${summary}` : "Brak konfiguracji wnętrza"}
          </p>
          <Button
            variant="outline"
            size="sm"
...
const CountertopConfigSection = ({
  ...
  const handleConfigChange = (config: CabinetCountertopConfig) => {
    ...
    // Check if excludeFromGroup is being enabled
    const excludeFromGroupChanging =
      config.excludeFromGroup === true && !countertopConfig?.excludeFromGroup;
    ...
    if (materialChanging && isPartOfGroup && countertopGroup) {
      ...
      setDialogOpen(false);
    } else if (excludeFromGroupChanging && isPartOfGroup) {
      // User wants to separate this cabinet - call separateCabinetFromGroup
      // First update the cabinet config
      onUpdateParams({ ...params, countertopConfig: config } as KitchenCabinetParams);
      // Then separate from group (this will regenerate countertops)
  ...
  const currentMaterialName =
    countertopMaterials.find((m) => m.id === countertopConfig?.materialId)?.name || "Nie wybrano";
      ...
      <AccordionContent className="pb-4 pt-0">
        <div className="space-y-3 px-1">
          <div className="text-xs text-muted-foreground space-y-1">
            <p>
              Materiał: <span className="text-foreground">{currentMaterialName}</span>
            </p>
            {isPartOfGroup && (
...
function CabinetPropertiesPanel({
  ...
  const cabinetRotation: [number, number, number] = selectedCabinet.worldTransform?.rotation ?? [
    0, 0, 0,
  ];
  ...
  const drawerHasFronts = localParams.drawerConfig
    ? Drawer.hasExternalFronts(localParams.drawerConfig)
    : false;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 pb-2 border-b bg-card z-10">
          ...
          <button
            ...
            onClick={() =>
              updateCabinet(selectedCabinet.id, {
                hideFronts: !(selectedCabinet.hideFronts ?? false),
              })
            }
            className={cn(
        ...
        {/* Hide countertops toggle - for kitchen and corner cabinets */}
        {(selectedCabinet.type === "KITCHEN" ||
          selectedCabinet.type === "CORNER_INTERNAL" ||
          selectedCabinet.type === "CORNER_EXTERNAL") && (
          <div className="flex items-center justify-between py-1">
            ...
            <button
              ...
              onClick={() =>
                updateCabinet(selectedCabinet.id, {
                  hideCountertops: !(selectedCabinet.hideCountertops ?? false),
                })
              }
              className={cn(
                "p-0.5 rounded hover:bg-muted/50 transition-colors",
      ...
      <div className="flex-grow overflow-y-auto px-2 py-2">
        <Accordion type="multiple" defaultValue={["dimensions"]} className="w-full space-y-1">
                ...
                <AccordionContent className="pb-4 pt-1 border-t mt-1">
                  <div className="flex items-center justify-between pb-3">
                    <div className="space-y-1">
                      ...
                      {!hasFrontsEnabled && (
                        <p className="text-[11px] text-muted-foreground">Fronty są wyłączone.</p>
                      )}
          ...
          {/* Countertop Config - for kitchen and corner cabinets */}
          {(selectedCabinet.type === "KITCHEN" ||
            selectedCabinet.type === "CORNER_INTERNAL" ||
            selectedCabinet.type === "CORNER_EXTERNAL") && (
            <div className="border rounded-md px-2 bg-card">
...
export function PropertiesPanel() {
  ...
  // Multiselect actions
  const { deleteSelectedParts, duplicateSelectedParts, updatePartsBatch, clearSelection } =
    useStore(
      useShallow((state) => ({
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="p-3 bg-muted/30 rounded-md border">
          ...
          <ul className="text-xs space-y-1 max-h-32 overflow-y-auto">
            {selectedParts.map((p) => (
              <li key={p.id} className="truncate text-foreground/80">
                • {p.name}
              </li>
            ))}
          </ul>
        ...
        <div className="space-y-2">
          ...
          <Select value={commonMaterialId ?? ""} onValueChange={handleBatchMaterialChange}>
        ...
        <Button variant="outline" size="sm" className="w-full h-8" onClick={clearSelection}>
          {t("clearSelection")}
        </Button>
      </div>
    );
  ...
  if (selectedPart) {
    ...
    const handleEdgeBandingUpdate = (edge: "top" | "bottom" | "left" | "right", value: boolean) => {
      if (selectedPart.shapeType === "RECT") {
        const currentEdgeBanding = (selectedPart.edgeBanding as EdgeBandingRect) || {};
      ...
      <div className="p-4 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-grow overflow-y-auto pr-1">
          <Accordion type="multiple" defaultValue={["basic", "shape"]} className="w-full space-y-1">
            ...
            {(selectedPart.cabinetMetadata?.role === "DOOR" ||
              selectedPart.cabinetMetadata?.role === "DRAWER_FRONT") && (
                  ...
                  <div className="pt-2">
                    <HandleSelector
                      ...
                      onChange={(handleConfig: HandleConfig | undefined) => {
                        ...
                        if (
                          doorMeta?.hingeSide === "LEFT" &&
                          selectedPart.name?.toLowerCase().includes("lewy")
                        ) {
                          doorType = "DOUBLE_LEFT";
                        } else if (
                          doorMeta?.hingeSide === "RIGHT" &&
                          selectedPart.name?.toLowerCase().includes("prawy")
                        ) {
